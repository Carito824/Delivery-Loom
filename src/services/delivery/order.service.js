    class OrderService {
      ...
      async create(data) {
        try {
          if (!data || !data.items_id || !data.total || !data.address) {
            return this._utilities.io.response.error('Please provide items_id, total and address');
          }
          ...
          const entity = new this._models.OrderModel(data, this._dependencies);
        ...
        try {
          ...
          switch (data.queryselector) {
            case 'id':
              ...
              break;
            case 'items_id':
              response = await this.#getByItems_id(data);
              break;
            case 'total':
              response = await this.#getByTotal(data);
              break;
            case 'address':
                response = await this.#getByAddress(data);
                break;
              case 'all':
      ...
      async #getByItems_id(data) {
            try {
              ...
              return this.#getByFilters({
                  filters: [
                    { key: 'items_id', operator: '==', value: data.search }  
                  ]
                });
      ...
      async #getByTotal(data) {
                try {
                  ...
                  return this.#getByFilters({
                      filters: [
                        { key: 'total', operator: '==', value: data.search }  
                      ]
                    });
      ...
      async #getByAddress(data) {
                  try {
                    ...
                    return this.#getByFilters({
                        filters: [
                          { key: 'address', operator: '==', value: data.search }  
                        ]
                      });
      ...
      async #getAll(data) {
                ...
                { key: 'status.name', operator: 'not-in', value: data.exclude_status },
    ...
    get status() {
    return this._models.OrderModel.statuses;
  }
  }
