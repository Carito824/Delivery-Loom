class MenuService {
  ...
  async create(data) {
    try {
      if (!data || !data.name || !data.description) {
        return this._utilities.io.response.error('Please provide name and description');
      }
      ...
      const entity = new this._models.MenuModel(data, this._dependencies);
    ...
    try {
      ...
      switch (data.queryselector) {
        ...
        case 'name':
          ...
          break;
          case 'description':
          response = await this.#getByDescription(data);
          break;
        case 'all':
  ...
  async #getByDescription(data) {
            try {
              ...
              return this.#getByFilters({
                  filters: [
                    { key: 'description', operator: '==', value: data.search }  
                  ]
                });
...
get status() {
return this._models.MenuModel.statuses;
}
}
